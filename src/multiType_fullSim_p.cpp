...
struct Parameters
{
    ...
    std::vector<double> k_vec; // drug efficacy factor / fitness cost in presence of drug
    double b; // baseline birth probability
    ...
    Parameters(int argc, char* argv[])
    {
        ...
        size_t conf_length = 15; //number of model input parameters
        ...
        b = std::stod(conf_v[14]); // baseline birth probability
...
struct treeData {
    ...
    int N_tot; // total number of individuals in the simulation
    ...
    while(retry_count < max_retries){
        ...
        for (int T = 1; T <= p.T_max; T++) {
            ...
            for (int type = 0; type < p.n_classes; ++type) {
                ...
                // Add offspring to next generation and total count
                data.N_tot += n_reproduce - n_del;
...
std::vector<double> birthProbs(const Parameters& p, int treat_indicator){
    ...
    // declare c variables for usage in lambda helper functions
    double c = p.fit_cost; // birth rate cost per mutation
    ...
    // converts individual reproductive number into birth probability
    auto r_to_b = [](double r) -> double {
        double b_star = 1 - ( 1/(1+r) );
        return b_star;
    };
    
    // converts birth probability into individual reproductive number
    auto b_to_r = [](double b) -> double {
        double r_star = ( 1/(1-b) ) - 1;
        return r_star;
    };
    
    // reduction of birth from fitness costs by # mutations
    auto fit_cost_r = [&c](double r, int n_mut) -> double {
        double r_star = r*pow((1-c), n_mut);
        return r_star;
    };
    
    // reduction of birth from drug induced mortality
    auto drug_cost_r = [](double r, double k) -> double {
        double r_star = r*(1-k);
        return r_star;
    };
    
    
    std::vector<double> birthProbs(p.n_classes, 0.0);

    double r0 = b_to_r(p.b);

    for (int cls = 0; cls < p.n_classes; cls++) {

        int cls_mut = numMutations(cls);
        double r_mut = fit_cost_r(r0, cls_mut);
        double b_mut = r_to_b(r_mut);

        double min_prob = b_mut;

        for (int drugType = 0; drugType < p.n_drugs; drugType++) {

            double prob; // comparison variable to get min bitrh probability

            // check for resistance to current drug type
            if ((cls >> drugType) & 1) {
                prob = b_mut; // resistant, so no change
                
            // only apply drug effects during treatment phase
            } else if (treat_indicator == 1) {
                double r_drug = drug_cost_r(r_mut, p.k_vec[drugType]);
                prob = r_to_b(r_drug); // susceptible, so apply additional reduction to R0
            } else {
                prob = b_mut; // no treatment so no change
            }
...
int main(int argc, char* argv[]){
    ...
    if (!simResults.is_open()) {
        std::cerr << "Unable to open results file: " << fname << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (!simFullResults.is_open()) {
        std::cerr << "Unable to open full teajectory file: " << full_fname << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if(p.data_out == 3){
        // column headers
        simResults << "rep;" << "ext;" << "crit;" << "treatSuccess" <<  std::endl;
        ...
        for(int i = 0; i < n_reps; i++){
            ...
            simResults << i+1 << ";" << res_out.extinct << ";" << res_out.crit << ";" << res_out.treatSuccess << std::endl;
        }
